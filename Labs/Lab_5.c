...
/***************************
*V10 k(omega)POT Function
****************************/
int Vk10()
{
        long x;             //Result
        long adHigh;        //High number
        long adLow;         //Low number
        int Voltage;       //Result for Voltage
        int V;              //Voltage number integer
        int V2;             //Voltage number remainder
    
        ADCON0 = 0b01111001;    //Register 1
        ADCON1 = 0b11000000;    //Register 2
        ...
        adLow = ADRESL;         //High register
        adHigh = ADRESH;        //Low register
        
        adHigh = adHigh << 8;       //Shift to the left by 8
        x = adHigh + adLow;         //Add the high and low registers

        Voltage = (x * 50)/1023;    //Voltage formula
        V = Voltage / 10;       //Gets the integer
        V2 = Voltage % 10;
...
int VPOT1()
{
        ...
        int V;              //Voltage number integer
        int V2;             //Voltage number remainder
        int V3;             //Voltage number remainder

        ADCON0 = 0b01100001;    //Register 1
        ADCON1 = 0b11000000;    //Register 2
        ...
        Voltage = (x * 98)/1023 + 2;    //Voltage formula
        volt = Voltage;
        V = Voltage / 10;       //Gets the integer
        V2 = Voltage % 10;      //Gets the remainder
        V3 = Voltage % 100;     //Gets the remainder
        ...
        Str_2[4] = 'Q';
        Str_2[5] = ' ';
        Str_2[6] = (Voltage / 100) + 48;
        Voltage = Voltage % 100;
        Str_2[7] = (Voltage / 10) + 48;
        Voltage = Voltage % 10;
        Str_2[8] = Voltage + 48;
 ...
 *DAC Function
 ***************************/
void DAC(double k10,int POT1)
{ 
    unsigned char controlByte = 0b00100001;     //Control byte
    long dataByte = 0;    
    double i;                                   //TIme in for loop
    double sinewave;                            //Result of sinewave
    int frequency;                              //Frequency in sinewave
    double amplitude;                           //Amplitude in sinewave
    
    

    TRISCbits.RC5 = 0;                          //Setting pin for RC5
    TRISCbits.RC0 = 0;                          //Setting pin for RC0
    

    for(i = 0; i < 1; i = i + 0.001)            //for loop that increments by 0.001
    {
        frequency = POT1;                       //Frequency from POT1 function
        amplitude = k10;                        //Amplitude from k10 function

        

//            dataByte = sinewave;

        PORTCbits.RC5 = 1;                      //Start 
        PORTCbits.RC0 = 0;

        SSPBUF = 0b00100001;                    //SSPBUF contains data from COntrol byte
        while(SSPSTATbits.BF == 0);
        
        sinewave = amplitude * sin((2*(3.14159)) * frequency * i);  //Convert sinewave to 255V
        sinewave = 255 * (sinewave + 5) / 10;

        SSPBUF = sinewave;
        while(SSPSTATbits.BF == 0);

        PORTCbits.RC5 = 0;
        PORTCbits.RC0 = 1;                      //Finish
    }
...
void main(void)
{
    ...
    TRISEbits.RE2 = 1;      //TRISC register for E2
    TRISAbits.RA3 = 0;      //TRISC register for A3
    TRISAbits.RA2 = 0;      //TRISC register for A2
    TRISAbits.RA1 = 0;      //TRISC register for A1
    SSPSTAT = 0b11000000;   //DAC SFR
    SSPCON1 = 0b00100000;   //DAC SFR
    ...
    while(1)
    {    
        double k10 = Vk10()/10;      //Result from function Vk10()
        int POT1 = VPOT1();    //Result from function VPOT1()
        DAC(k10,POT1);          //DAC result
