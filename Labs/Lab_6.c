...
#include <time.h>   //For clock programming
...
void Digital_Clock();   //Function for a digital clock
...
void Digital_Clock() 
{
        ...
        if(second <= 9)
        {
            Str_1[8] = second + 48;     //00:00:0#  Ex: 00:00:01
        }
        
        if((second >= 10) && (second <= 59))
        {
            Str_1[8] = (second % 10) + 48;      //00:00:##  Ex: 00:00:59
            Str_1[7] = (second/10) + 48;
        }
        
        if(second == 60)
        {
            minute += 1;                //00:0#:00  Ex: 00:01:00
            second = 0;
        }
        ...
        if(minute <= 9)
        {
            Str_1[5] = minute + 48;     //00:0#:##  Ex: 00:01:59
        }
        
        if((minute >= 10) && (minute <= 59))
        {
            Str_1[5] = (minute % 10) + 48;  //00:#0:00  Ex: 00:10:00
            Str_1[4] = (minute/10) + 48;
        }
        
        if(minute == 60)
        {
            hour += 1;                      //0#:00:00  Ex: 01:00:00
            minute = 0;
        }
        ...
        if(hour <= 9)
        {
            Str_1[2] = hour + 48;       //0#:##:##      Ex: 01:59:59
        }
        
        if((hour >= 10) && (hour <= 23))
        {
            Str_1[2] = (hour % 10) + 48;    //##:##:##  Ex: 23:59:59
            Str_1[1] = (hour/10) + 48;
        }
...
{        
    ...
    if((INTCONbits.INT0E == 1) && (INTCONbits.INT0F == 1))  //Reset clock
    {
        hour = 0;
        minute = 0;
        second = 0;
        ...
        TMR0H = 0x67;               //Preload High
        TMR0L = 0x69;               //Preload Low
        INTCONbits.INT0IF = 0;      //Clear INT0 Flag bit (reset))
    }
}
/***************************
 * Low Interrupt
...
void interrupt low_priority My_ISR_Low(void)
{
    if(INTCON3bits.INT1IF == 1)
    {
        ...
        {
            T0CONbits.TMR0ON = 1;   //Turn Timer 0 off
            timer = 0;
        }
        INTCON3bits.INT1IF = 0;
    }
    
    if(INTCONbits.TMR0IF == 1)      //Start the clock
    {
        INTCONbits.TMR0IF = 0;
...
void main(void)
{
    ...
    TRISBbits.RB0 = 1;      //TRISC register for B0
    TRISBbits.RB1 = 1;      //TRISC register for B1
    TRISBbits.RB2 = 1;      //TRISC register for B2
    TRISBbits.RB3 = 1;      //TRISC register for B3
    
    RB0 = PORTBbits.RB0;    //Setting RB0
    RB1 = PORTBbits.RB1;    //Setting RB1
    RB2 = PORTBbits.RB2;    //Setting RB2
    RB3 = PORTBbits.RB3;    //Setting RB3
    
    T0CON = 0b00000101;     //TImer 0
    INTCON = 0b00110000;    //INTCON
    INTCON2 = 0b11110000;   //INTCON 2
    INTCON3 = 0b00001000;   //INTCON 3
    RCON = 0b10000000;      //RCON
    
    INTCONbits.GIE = 1;     //INTCON GIE
    INTCONbits.PEIE = 1;    //INTCON PEIE
    
    T0CONbits.TMR0ON = 1;       //Starts Timer 0
